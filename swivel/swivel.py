...
flags.DEFINE_float(
    'row_col_vector_binding_factor', 0.01,
    'vector regularization sensitivity parameter see https://arxiv.org/pdf/1606.09403')
...
class Model(object):
  ...
  def __init__(self, input_base_path, hparams):
    ...
    l2_loss = loss_multiplier * tf.reduce_sum(
        0.5 * l2_confidence * tf.square(err)) # should we multiply by count_is_nonzero?

    sigmoid_loss = loss_multiplier * tf.reduce_sum(
        tf.nn.softplus(err) * count_is_zero)

	# TODO add a vector_regul_loss? 
  ...
  def _write_tensor(self, vocab_path, output_path, session, embedding):
    ...
    with open(output_path, 'w', encoding='utf_8') as out_f:
      with open(vocab_path, encoding='utf_8') as vocab_f:
