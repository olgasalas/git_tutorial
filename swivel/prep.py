...
flags.DEFINE_integer('max_vocab', 4096 * 256, 'The maximum vocabulary size')
...
def write_vocab_and_sums(vocab, sums, vocab_filename, sums_filename):
  ...
  with open(os.path.join(FLAGS.output_dir, vocab_filename), 'w', encoding='utf_8') as vocab_out:
    with open(os.path.join(FLAGS.output_dir, sums_filename), 'w', encoding='utf_8') as sums_out:
...
def compute_coocs(lines, vocab):
  ...
  for lineno, line in enumerate(lines, start=1):
    ...
    for lid in wids:
      last_lid = lid
      #lid = wids[pos]
      window_extent = min(FLAGS.window_size + 1, len_wids - pos)
    ...
    if lineno % 10000 == 0:
      ...
      #    100.0 * pos / nbytes, pos, nbytes))
      sys.stdout.write('\rComputing co-occurrences: %d..., last lid %s, sum(%s)=%f' % (lineno, last_lid, last_lid, sums[last_lid]))
      sys.stdout.flush()
...
def main(_):
  ...
  if FLAGS.vocab:
    with open(FLAGS.vocab, 'r', encoding='utf_8') as lines:
      vocab = [line.strip() for line in lines]
  else:
    with open(FLAGS.input, 'r', encoding='utf_8') as lines:
